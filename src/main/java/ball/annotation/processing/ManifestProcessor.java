...
@For({ Attribute.class, MainClass.class, Section.class, JavaBean.class, DependsOn.class, DesignTimeOnly.class })
...
public class ManifestProcessor extends AnnotatedProcessor implements ClassFileProcessor {
    ...
    protected void process(RoundEnvironment roundEnv, TypeElement annotation, Element element) {
            ...
            case INTERFACE:
                ...
                if (method != null && (method.getModifiers().containsAll(getModifiers(PROTOTYPE)))) {
                    ...
                    print(ERROR, element,
                          ...
                          main.annotationType().getSimpleName(), element.getKind(), declaration(PROTOTYPE));
                }
                break;
        ...
        DesignTimeOnly design = element.getAnnotation(DesignTimeOnly.class);
    }
    ...
    public void process(Set<Class<?>> set, JavaFileManager fm) throws Exception {
        ...
        for (Class<?> type : set) {
            URI uri = fm.getJavaFileForInput(CLASS_OUTPUT, type.getName(), CLASS).toUri();
    ...
    private class ManifestImpl extends Manifest {
        ...
        public String put(MainClass main, String name) {
            return getMainAttributes().putValue(getAttributeName(main.getClass()), name);
        }
        ...
        public void put(String path, JavaBean bean, DependsOn depends, DesignTimeOnly design) {
            Attributes attributes = getEntries().get(path);
            ...
            attributes.putValue(getAttributeName(JavaBean.class), String.valueOf(true));

            if (depends != null) {
                attributes.putValue(getAttributeName(depends.getClass()), String.join(" ", depends.value()));
            }

            if (design != null) {
                attributes.putValue(getAttributeName(design.getClass()), String.join(" ", design.value()));
            }
        }
